...
const relatedComments = computed(() =>
  commentsStore.comments.filter((comment:ICommentItem) => comment.newsId === newsId)
)
</script>
...
<template>
  <div class="container py-4">
    <div v-if="newsItem" class="mb-5">
      ...
      <p class="text-muted">{{ new Date(newsItem.date).toLocaleDateString() }}</p>
    ...
    <div v-if="relatedComments.length">
      ...
      <ul class="list-group">
        <li v-for="comment in relatedComments" :key="comment.id" class="list-group-item">
          <strong>{{ comment.author }}</strong>
          <span class="text-muted ms-2">({{ new Date(comment.date).toLocaleDateString() }})</span>
